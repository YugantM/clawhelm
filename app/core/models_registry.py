...
from ..services.performance import get_successful_models
...
class ModelRegistry:
    ...
    def _register_model(self, model_id: str, *, provider: str, is_free: bool, source: str) -> None:
        self._models[model_id] = ModelInfo(id=model_id, provider=provider, is_free=is_free, source=source)
    ...
    async def _fetch_openrouter_models(self, client: httpx.AsyncClient) -> None:
        ...
        for item in data:
            ...
            if not isinstance(model_id, str):
                continue
            self._register_model(model_id, provider="openrouter", is_free=model_id.endswith(":free"), source="fetched")
